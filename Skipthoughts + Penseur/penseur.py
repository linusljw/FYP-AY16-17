...
class Penseur:
	...
	def get_final(self):
		...
		with open('input.txt') as input, open('output.txt') as output:
			for input_line, output_line in izip(input,output):
				inputList = []
				inputList.extend(self.get_closest_sentences(input_line, 20000))
				x = output_line
				if x in inputList:
					str_x = str(inputList.index(x) + 1)
					result.write(str_x + "\n")
				else:
					result.write("None" + "\n")
	...
	def get_recall(self):
		...
		with open('input.txt') as input, open('output.txt') as output:
			for input_line, output_line in izip(input,output):
				...
				recall5List = []
				recall10List = []
				recall1List.extend(self.get_closest_sentences(input_line, 1))
				recall5List.extend(self.get_closest_sentences(input_line, 5))
				recall10List.extend(self.get_closest_sentences(input_line, 10))
				x = output_line
				if x in recall1List:
					result.write("Recall @ 1 = 1" + "\n")
				elif x in recall5List:
					result.write("Recall @ 5 = 1" + "\n")
				elif x in recall10List:
