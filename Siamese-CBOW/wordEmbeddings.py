...
class wordEmbeddings:
  ...
  def get_cosine(self):
	...
	for i1 , o1 in zip(input,output):
	 	cosine_s = self.sentence_similarity(i1, o1)
		cosine_simi.write(str(cosine_s) + '\n')

  def get_final(self):
	result = open('results.txt' , 'a') 
	X = open('output.txt').readlines()
	with open('input.txt') as input , open ('cosine.txt') as cosine:
		for input_line, cosine_line in izip(input,cosine):
			counter = 1
			for output_line in X:
				sentence_simi = self.sentence_similarity(input_line , output_line)
				if sentence_simi > float(cosine_line):
					counter += 1
		...
		for input_line, cosine_line in izip(input,cosine):
			...
			for output_line in X:
				...
				if sentence_simi > float(cosine_line):
					recall_counter += 1
			if recall_counter == 1:
				result.write("Recall @ 1 = 1" + '\n')
			elif recall_counter > 1 and recall_counter <= 5:
				result.write("Recall @ 5 = 1" + '\n')
			elif recall_counter > 5 and recall_counter <= 10:
				result.write("Recall @ 10 = 1" + '\n')
			else:
				result.write("None" + '\n')
